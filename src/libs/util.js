...
const backendMenuToRoute = (menu) => {
  ...
  let route = {}
  route.path = menu.path.indexOf('/') === 0 ? menu.path : `/${menu.path}`
  route.name = menu.menuCode
  route.meta = {
    icon: menu.menuIcon,
    title: menu.menuLabel,
    notCache: false
    // menuOpenMode: 1侧边菜单 0隐藏菜单
    // hideInMenu: !menu.menuOpenMode
  }
 ...
 * @returns {Array} 其中的每个元素只包含路由原信息中的name, path, meta三项
 */
export const getRouterListInLocalstorage = () => {
...
/**
 * @param {*} list 现有标签导航列表
 * @param {*} newRoute 新新增的路由原信息对象
 * @description 如果该newRoute已经存在则不再新增
 */
