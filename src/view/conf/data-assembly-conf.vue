  ...
  <div class="main">
    ...
    <Modal
      ...
      <Form :model="formItem" :label-width="100" :rules="ruleValidate" ref="formValidate">
        ...
        <FormItem label="数据源" prop="configKafka">
           <Select v-model="formItem.configKafka" @on-change="configKafkaSelectChange">
              <Option :value="item.id" v-for="item in configKafkaList" :key="item.id">{{item.name}}</Option>
            </Select>
        </FormItem>
        <FormItem label="检索数据标记" prop="searchDataFlag">
          <Input v-model="formItem.searchDataFlag" disabled></Input>
        </FormItem>
...
export default {
    ...
    return {
      ...
      columns: [{
        ...
        render: function (h, params) {
          return h('span', params.row.configKafkaInfo ? params.row.configKafkaInfo.name : params.row.configKafka)
        }
      }, {
        title: '检索数据标记',
        align: 'center',
  ...
  methods: {
    ...mapActions([
      ...
      'addDataAssemblyConf',
      'removeDataAssemblyConf',
      'updateDataAssemblyConf',
      'getDictTypesWithCode',
      'getKafkaConfList'
    ]),
    configKafkaSelectChange (val) {
      const item = this.configKafkaList.find(item => item.id === val)
      if (item) {
        this.formItem.searchDataFlag = item.searchDataFlags
      }
    },
    // 获取字典项
    getDictTypeList () {
      this.getDictTypesWithCode('data_pack_type').then(res => {
    ...
    getKafkaConfigList () {
      this.getKafkaConfList({
        ...
        if (res.data.code === 'success') {
          this.configKafkaList = res.data.data.records
        }
      }).catch(err => {
        console.error(err)
      })
    ...
    getData () {
      ...
      this.getDataAssemblyConfList(this.query).then(res => {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.modal.isEdit) {
            this.updateDataAssemblyConf(this.formItem).then(res => {
              this.modal.isShow = false
              this.getData()
            })
          } else {
            this.addDataAssemblyConf(this.formItem).then(res => {
              this.modal.isShow = false
              this.getData()
            })
          }
